	...
	tests := []struct {
		...
		},
		{
			...
			messages: []llms.MessageContent{
				{
					Parts: []llms.ContentPart{
						llms.TextContent{Text: "Test error"},
					},
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mockClient := &mockHTTPClient{
				response: &http.Response{
					StatusCode: 200,
					Body:       newMockBody(tt.response),
				},
			}

			if tt.wantErr {
				mockClient.response.StatusCode = 400
			}
			...
			resp, err := llm.GenerateContent(context.Background(), tt.messages)
			if (err != nil) != tt.wantErr {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantError {
				t.Errorf("GenerateContent() error = %v, wantError %v", err, tt.wantError)
			}

			if handler.generateStartCalled != tt.wantStarted {
				t.Errorf("HandleLLMGenerateContentStart called = %v, want %v", handler.generateStartCalled, tt.wantStarted)
			}
			if handler.generateEndCalled != tt.wantEnded {
				t.Errorf("HandleLLMGenerateContentEnd called = %v, want %v", handler.generateEndCalled, tt.wantEnded)
			}
			if handler.errorCalled != tt.wantErrored {
				t.Errorf("HandleLLMError called = %v, want %v", handler.errorCalled, tt.wantErrored)
			}
		})
	}
}
