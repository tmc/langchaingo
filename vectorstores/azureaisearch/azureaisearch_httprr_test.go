...
func TestStoreHTTPRR_CreateIndex(t *testing.T) {
	...
	indexOptions := []IndexOption{
		WithVectorLength(4),
		WithSemanticConfiguration("default"),
		WithSimilarity("cosine"),
	}

	err = store.CreateIndex(context.Background(), indexName, indexOptions...)
	require.NoError(t, err)
}
...
func TestStoreHTTPRR_DeleteIndex(t *testing.T) {
	...
	err = store.DeleteIndex(context.Background(), indexName)
	require.NoError(t, err)
}
...
func TestStoreHTTPRR_ListIndexes(t *testing.T) {
	...
	indexes, err := store.ListIndexes(context.Background(), nil)
	require.NoError(t, err)
