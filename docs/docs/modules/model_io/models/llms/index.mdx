...
- **Truncation**: `llms.WithFailOnTruncation` turns an answer cut short by the output-token limit into a typed error instead of a silently shortened result; the partial answer is returned alongside it, and `ContentChoice.Truncated` carries the same signal without the option.
