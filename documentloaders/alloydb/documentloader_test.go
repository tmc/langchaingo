...
func setUpEngine(t *testing.T) (alloydbutil.PostgresEngine, func(), error) {
	t.Helper()
	...
	if username == "" || password == "" || database == "" || projectID == "" || region == "" || cluster == "" || instance == "" {
		...
		if err != nil {
			return alloydbutil.PostgresEngine{}, nil, err
		}

		pool, err := pgxpool.New(ctx, container.URI)
		if err != nil {
			return alloydbutil.PostgresEngine{}, nil, fmt.Errorf("failed to instantiate pgx pool: %w", err)
		}
...
func setup(t *testing.T) (alloydbutil.PostgresEngine, func(), error) {
	t.Helper()
...
func TestNewDocumentLoader_Fail(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "invalid engine",
			setDocumentLoader: func() (*DocumentLoader, error) {
				return NewDocumentLoader(context.Background(), alloydbutil.PostgresEngine{})
			},
			validateFunc: func(t *testing.T, d *DocumentLoader, err error) {
				t.Helper()
				assert.Nil(t, d)
...
func createTable(t *testing.T, testEngine alloydbutil.PostgresEngine) {
	t.Helper()

	err := testEngine.InitVectorstoreTable(context.Background(), alloydbutil.VectorstoreTableOptions{
		TableName:         "testtable",
		VectorSize:        3,
		SchemaName:        "public",
...
func insertRows(t *testing.T, testEngine alloydbutil.PostgresEngine) {
	t.Helper()
